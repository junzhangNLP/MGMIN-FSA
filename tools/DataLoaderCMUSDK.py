...
from tools.Utils import whether_type_str
...
def multi_collate_mosei_mosi(batch):
    ...
    input_ids = torch.stack([sample[3] for sample in batch])
    att_mask = torch.stack([sample[4] for sample in batch])
    # get the data out of the batch - use pad sequence util functions from PyTorch to pad things
    labels = torch.Tensor([sample[5] for sample in batch]).reshape(-1,).float()
    labels_2 = torch.Tensor([sample[6] for sample in batch]).reshape(-1,).long()
    ...
    return sentences, acoustic, visual, input_ids, att_mask,labels, labels_2, lengths
...
def get_mosi_dataset(mode='train', text='glove', audio='covarep', video='facet42', normalize=[True, True, True]):
    ...
    text_bert_features = [' '.join(np.nan_to_num(data_[0][0][mosi_l_features.index("text")], nan=0.0, posinf=0, neginf=0)) for data_ in data]
    input_ids, att_mask = bert_inputs(text_bert_features)
    if normalize[0]:
    ...
    return l_features, a_features, v_features, input_ids, att_mask, labels, labels_2
...
def get_mosei_dataset(mode='train', text='glove', audio='covarep', video='facet42', normalize=[True, True, True]):
    ...
    return l_features, a_features, v_features, input_ids, att_mask, labels, labels_2, labels_7
...
class CMUSDKDataset(Dataset):
    def __init__(self, mode, dataset='mosi', text='glove', audio='covarep', video='facet42', normalize=[True, True, True]):
        ...
        if dataset == 'mosi':
            self.l_features, self.a_features, self.v_features,self.input_ids, self.att_mask, self.labels, self.labels_2 = get_mosi_dataset(mode=mode, text=text, audio=audio, video=video, normalize=normalize)
        elif dataset == 'mosei':
            self.l_features, self.a_features, self.v_features,self.input_ids, self.att_mask, self.labels, self.labels_2, self.labels_7 = get_mosei_dataset(mode=mode, text=text, audio=audio, video=video, normalize=normalize)
        else:
            raise NotImplementedError

    def __getitem__(self, index):        
        if self.dataset == 'mosi':
            return self.l_features[index], self.a_features[index], self.v_features[index], self.input_ids[index], self.att_mask[index], self.labels[index], self.labels_2[index]
        elif self.dataset == 'mosei':
            return self.l_features[index], self.a_features[index], self.v_features[index], self.input_ids[index], self.att_mask[index], self.labels[index], self.labels_2[index], self.labels_7[index]
        else:
            raise NotImplementedError
